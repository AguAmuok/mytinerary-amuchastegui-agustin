...
const activitiesActions = { //action es un objeto que contiene una funcion con dos propiedades
    ...
    getActivities: () => {
        return async (dispatch, getState) => { // el dispatch es lo que recibe como parametro el reducer junto al estado para luego trabajarlo y cambiarlo
            const res = await axios.get('https://mi-tinerary-agustin-amu-back.herokuapp.com/api/activities')
    ...
    getOneActivity: (id) => {
        ...
        const res = await axios.get(`https://mi-tinerary-agustin-amu-back.herokuapp.com/api/activities/${id}`)
...
getActivitiesByitinerary: (id) => { //muestra itinerarios por ID
                return async (dispatch, getState) => {
        const res = await axios.get(`https://mi-tinerary-agustin-amu-back.herokuapp.com/api/activitiesbyitinerary/${id}`)
